...
<<<<<<< HEAD
```{r}
ams <- readRDS("/Users/soizicdieulouard/Downloads/database_ams_a_b_c.rds")
=======
## Gestions des données

```{r, message=FALSE}
ams <- readRDS("Data/database_ams_a_b_c.rds")
>>>>>>> d93a25fa33a65da5220dc7aee315619fc900025a
head(ams)
str(ams)
...
Individus qui changeraient de groupe CPAP au cours du temps:
...
#53 individus passe de pas CPAP à CPAP
```
...
m2complet <- lme(fixed= SCHRAG_TOT ~ delai_consult_vis0*(CPAP + UMSARS1and2_TOT + UMSARS4 + delaiDecesCensor + SPT1DYS + SEXE), random = ~ delai_consult_vis0 | id, data= ams, method='ML', na.action=na.omit)
AIC(m2complet)
...
Si on essaye de faire un modèle avec UMTOT :
...
LME chez ceux qui changent de groupe (cpap 0 à cpap 1)
